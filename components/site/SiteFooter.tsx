    ...
    <div className="container-site py-14">
      <div className="grid gap-10 md:grid-cols-[1.5fr_1fr_1fr]">
        ...
        <nav><p className="vow-label">{sw?"Sheria":"Legal"}</p><ul className="mt-5 space-y-3 text-sm">
          <li><Link to="/privacy-policy">{sw?"Sera ya Faragha":"Privacy Policy"}</Link></li><li><a href="/terms">{sw?"Masharti / EULA":"Terms / EULA"}</a></li><li><a href="/copyright">{sw?"Hakimiliki / DMCA":"Copyright / DMCA"}</a></li><li><Link to="/support">{sw?"Kufuta akaunti":"Account deletion"}</Link></li>
        </ul></nav>
      </div>
      <div className="mt-12 border-t border-vow-border pt-6 text-xs text-vow-muted">
        <p>{sw?"© 2026 VOW. Haki zote zimehifadhiwa.":"© 2026 VOW. All rights reserved."}</p>
        <p className="mt-2">{sw?"Imeundwa na kutengenezwa na Joshua Nathan Kasanga":"Designed and Developed by Joshua Nathan Kasanga"}</p>
      </div>
    </div>
  </footer>;
}
